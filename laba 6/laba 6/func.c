...
int** findMaxElements(int** matrixOfNums, int* rows, int* columns) {
	int tempRows = 2;
	int** tempMatrix = (int**)malloc(tempRows * sizeof(int*));
	for (int i = 0; i < 2; i++) {
		tempMatrix[i] = (int*)malloc(*(columns) * sizeof(int));
	}
	for (int i = 0; i < *columns; i++) {
		tempMatrix[0][i] = matrixOfNums[0][i];
		tempMatrix[1][i] = i;
	}
		for (int j = 0; j < *columns; j++) {
			for (int i = 0; i < *rows; i++) {
				if (matrixOfNums[i][j] > tempMatrix[0][j]) tempMatrix[0][j] = matrixOfNums[i][j];
			}
		}
...
void shellSort(int** tempMatrix, int columns) {
	int buferVariable;
	int buferIndex;
	int i;
	for (int step = columns / 2; step != 0; step /=2) {
		for (i = 0; i < columns; i++) {
...
int** makeSortedMatrix(int** matrixOfNums, int** tempMatrix, int *rows, int *columns) {
	...
	for (int i = 0; i < *rows; i++) {
		...
		;
		for (int j = 0; j < *columns; j++) {
			element = tempMatrix[1][j];
			for (int i = 0; i < *rows; i++) {
			
			sortedMatrix[i][j] = matrixOfNums[i][element];
		}
	}
	return sortedMatrix;
}
